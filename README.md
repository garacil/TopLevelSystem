...
Connects Portal instances into a distributed network. Worker thread pool per peer for maximum throughput. Wire protocol over TCP/TLS. Remote paths appear locally as `/<node_name>/*`. Automatic reconnection with exponential backoff (1s→30s cap) — never gives up.
